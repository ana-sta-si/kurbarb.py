...
class Frame_drink(tk.Frame):
    ...
    def add_possition(self, button):
        for i in range(len(self.buttons)):
            if i == 0 and i == button:
                code_b = i + 1
                record_exists = MainApp.curs.execute('''SELECT Код_блюда, Количество FROM Карта_заказа WHERE Код_блюда = ?''',(code_b,)).fetchone()
                if record_exists:
                    MainApp.curs.execute('''UPDATE Карта_заказа SET Количество = ?  WHERE Код_блюда = ?''', (record_exists[1] + 1,  code_b))
                else:
                    MainApp.curs.execute('''INSERT  INTO  Карта_заказа(Название_блюда, Цена_блюда, Код_блюда) SELECT Название, Цена, Код_блюда FROM Меню WHERE Код_раздела = 1 and Код_блюда = ?''', (code_b,))
                    query = f"Update Карта_заказа set Количество = 1"
                ...
                code_b = i + 1
                record_exists = MainApp.curs.execute(
                    '''SELECT Код_блюда, Количество FROM Карта_заказа WHERE Код_блюда = ?''', (code_b,)).fetchone()
                if record_exists:
                    MainApp.curs.execute('''UPDATE Карта_заказа SET Количество = ?  WHERE Код_блюда = ?''',
                                         (record_exists[1] + 1, code_b))
                else:
                    MainApp.curs.execute(
                        '''INSERT  INTO  Карта_заказа(Название_блюда, Цена_блюда, Код_блюда) SELECT Название, Цена, Код_блюда FROM Меню WHERE Код_раздела = 1 and Код_блюда = ?''',
                        (code_b,))
                    query = f"Update Карта_заказа set Количество = 1"
                    MainApp.curs.execute(query)
                MainApp.conn.commit()
...
class Frame_zakaz(tk.Frame):
    # def resturt(self):
    #     Frame_zakaz.destroy(self)
    #     Frame_zakaz.create_table(self)
    def __init__(self, parent):
        super().__init__(parent)
        self.place(y=150, x=0, height=605, width=750)
        self.configure(background="white")
        self.tabel = self.create_table()
