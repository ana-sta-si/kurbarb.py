...
class MainApp(tk.Tk):
    def __init__(self):
        ...
        but_ok = tk.Button(self, background='#202020', text='Внести', command=lambda: self.in_(self.curr_time,self.entry_2.get()), font="Helvetica 25 bold", foreground='#F8F8FF', borderwidth=0)
        but_ok.place(x = 690, y = 730)
    def close(self):
        app.destroy()
    def in_(self, data, summa):
        conn = sqlite3.connect('DataBase.db')
        curs = conn.cursor()
        curs.execute(f'INSERT INTO Внесение (дата_время, сумма) VALUES ("{data}","{summa}")')
        conn.commit()
        curs.close()
        conn.commit()
        app.destroy()
