...
class MainApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title('Работа с меню') #название программы
...
class Frame1(tk.Frame):
    def __init__(self, parent):
        ...
        tree.heading("coder", text="Код раздела")
        MainApp.cursor.execute("SELECT * FROM Меню")
        result = MainApp.cursor.fetchall()
        for row in result:
            tree.insert('', tk.END, values=row)
        scroll_bar = tk.Scrollbar(self)
        scroll_bar.pack(side=tk.RIGHT, fill=tk.Y)
# фрейм для добавления блюд в меню
class Frame2(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent)
        self.place(y=0, x = 1151, height=1200, width=950)
        self.configure(background="grey")
        label_1 = tk.Label(self, text = "Добавить блюдо", font="Helvetica 47", foreground="black", background='grey')
        label_1.pack(pady=90, padx=100, anchor=tk.NW)
        list_label = ['Код', 'Название', "Цена", "Раздел"]
        ...
        self.combo = ttk.Combobox(self,values=["1", "2", "3", "4", "5","6"],font="Helvetica 27")
        self.combo.place(x=279, y=425)
        but_add = tk.Button(self, background='grey', text='Добавить', command= lambda: self.add(self.text_1.get(),self.text_2.get(),self.text_3.get(),self.combo.get()), font="Helvetica 27 bold", foreground='black', borderwidth=0)
    ...
    def add(self, code, name, price, coder):
        MainApp.cursor.execute(f'SELECT * FROM Меню WHERE Код = "{code}";')
        MainApp.result = MainApp.cursor.fetchone()
        if MainApp.result !=[]:
            MainApp.cursor.execute(f'INSERT INTO Меню (Код, Название, Цена, Раздел) VALUES ("{code}","{name}", "{price}","{coder}")')
            MainApp.conn.commit()
